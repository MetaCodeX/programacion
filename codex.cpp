...
class Tienda {
public:
    static int idTicketActual;

    // Método para generar un nuevo ticket
    void generarTicket(ModuloAlmacen& moduloAlmacen) {
        int idProducto, idVendedor, cantidad;
        string metodoPago, fechaHoraStr;
        ...
        cin >> cantidad;
        cout << "Ingrese el método de pago (efectivo, credito, debito, vale): ";
        cin >> metodoPago;
        cout << "Ingrese la cantidad de dinero disponible: ";
        ...
        fechaHora << put_time(ltm, "%Y-%m-%d %H:%M:%S");
        fechaHoraStr = fechaHora.str();

        // Validar datos
        if (!validarDatos(idProducto, idVendedor, cantidad, metodoPago, dineroDisponible, moduloAlmacen)) {
            return; // Salir si los datos son inválidos
        }

        // Obtener información del producto y aplicar ofertas
        Mercancia* mercancia = moduloAlmacen.buscarMercanciaPorId(idProducto);
        double precioUnitario = mercancia->precio;
        double precioTotal = calcularPrecioTotal(precioUnitario, cantidad, mercancia->idsOfertas, moduloAlmacen);

        // Calcular dinero sobrante
        double dineroSobrante = dineroDisponible - precioTotal;

        // Generar ID de ticket único
        int idTicket = generarIdTicket();

        // Escribir ticket en el archivo
        ofstream archivo("ticket.txt", ios::app);
        archivo << idTicket << "," << idVendedor << "," << idProducto << "," << mercancia->nombre << ","
                << precioUnitario << "," << cantidad << "," << metodoPago << "," << dineroDisponible << ","
                << precioTotal << "," << fechaHoraStr << "," << dineroSobrante << endl;
        archivo.close();

        cout << "Ticket generado correctamente con ID: " << idTicket << endl;
    }

    // Método para mostrar los tickets
    void mostrarTickets() {
        ifstream archivo("ticket.txt");
        if (!archivo.is_open()) {
            cerr << "Error al abrir el archivo de tickets." << endl;
            return;
        }

        string linea;
        cout << "Tickets de venta:\n\n";
        while (getline(archivo, linea)) {
            ...
            // Mostrar los campos del ticket
            cout << "ID Ticket: " << campos[0] << endl;
            cout << "ID Vendedor: " << campos[1] << endl;
            cout << "ID Producto: " << campos[2] << endl;
            cout << "Nombre Producto: " << campos[3] << endl;
            cout << "Precio Unitario: $" << stod(campos[4]) << endl; // Convertir a double
            cout << "Cantidad: " << campos[5] << endl;
            cout << "Método de Pago: " << campos[6] << endl;
            cout << "Dinero Disponible: $" << stod(campos[7]) << endl; // Convertir a double
            cout << "Precio Total: $" << stod(campos[8]) << endl; // Convertir a double
            cout << "Fecha y Hora: " << campos[9] << endl;
            cout << "Dinero Sobrante: $" << stod(campos[10]) << endl; // Convertir a double
            cout << "--------------------\n";
        }
...
private:
    // ... (Métodos generarIdTicket(), validarDatos() y calcularPrecioTotal() - sin cambios)
};

int Tienda::idTicketActual = 1;
